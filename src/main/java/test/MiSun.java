...
public class MiSun {
    public static void main(String[] args) {
        ...
        //회원관리 배열
        //계정코드가 administrator 일 경우 관리자 계정
        String[][] memberData = {
                {"admin", "admin1234", "administrator"},
                {"user", "user123",}
        };
        //id, passwd, 계정 코드(user 는 아직 미정)
            ...
            while (true) {//로그인 or 회원가입 선택
                ...
                switch (loginChoice) {
                    case 1://로그인
                        ...
                        }//로그인 시도 id가 관리자 id 인지 여부 판별 if문 종료
                ...
                while (true) {
                    ...
                    switch (menuInsert) {
                        case 1://신규도서 등록
                            ...
                            System.out.println("\n# 신규도서 등록을 시작합니다.");
                            sc.nextLine();
                            //책 제목, 저자, 출판사 등에 Space bar(공백문자)가 포함되어있는 경우가 있기 때문에 nextLine 으로 값을 입력받아야 함.
                            //위의 메뉴선택 과정에서 입력한 Enter 가 남아있기 때문에 nextLine 을 한줄 추가해서 제거해줌.
                            ...
                            for (int j = 0; j < listLength; j++) {
                                if (books[j][0].equals(changeNum)) {// 배열의 0번인덱스가 입력받은 책 번호와 같다면
                                    ...
                                    while (true) { //수정 항목 잘못 입력시 반복, 취소 입력시 반복문 탈출하도록 코드 작성
                                        ...
                                        System.out.println("\n- 어떤 정보를 수정하시겠습니까? 1. [제목], 2. [저자], 3. [출판사], 4. [가격], 5. 취소");
                                        System.out.print("> ");
